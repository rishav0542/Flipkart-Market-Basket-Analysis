# Flipkart-Market-Basket-Analysis
End-to-end Market Basket Analysis, Customer Segmentation &amp; Product Recommendation System for Flipkart using Python. Includes Apriori Algorithm, K-Means Clustering, association rules, and actionable business insights. 

# Flipkart Market Basket Analysis

**End-to-End Customer Insights & Product Recommendation System**

This project performs **Market Basket Analysis**, **Customer Segmentation**, and generates **actionable recommendations** for Flipkart using Python.

## 📌 Project Objective
To understand customer purchasing behaviour, identify frequently bought-together product categories, segment customers, and provide data-driven strategies to improve cross-selling, personalization, and customer retention.

## 🛠 Technologies Used
- Python
- Pandas, NumPy
- Seaborn, Matplotlib
- Scipy (K-Means)
- Itertools & Collections (Apriori)

## 📊 Key Insights from Analysis

### Market Basket Analysis (Apriori)
- **Clothing and Fashion** is the most central category with the strongest co-purchase relationships.
- Top association rules:
  - Clothing and Fashion + Beauty and Personal Care (Support: 0.278)
  - Clothing and Fashion + Home and Kitchen (Support: 0.266)
  - Clothing and Fashion + Groceries and Gourmet Food (Support: 0.261)

**Strongest bundle:** Clothing + Beauty products

## 📊 Key Visualizations

### 1. Customer Segment Sizes
![Customer Segment Sizes](FLIPKART%20MBA%20VISUALS/Segment%20Size.png)

### 2. Shopping Satisfaction Distribution (Pie Chart)
![Shopping Satisfaction Distribution](FLIPKART%20MBA%20VISUALS/Shopping%20Satisfaction%20Distribution.png)

### 3. Distribution of Purchase Frequency
![Distribution of Purchase Frequency](FLIPKART%20MBA%20VISUALS/Distribution%20of%20Purchase%20Frequency.png)

### 4. Distribution of Browsing Frequency
![Distribution of Browsing Frequency](FLIPKART%20MBA%20VISUALS/Distribution%20of%20Browsing%20Frequency.png)

### 5. Distribution of Purchase Categories
![Distribution of Purchase Categories](FLIPKART%20MBA%20VISUALS/Distribution%20of%20Purchase%20Categories.png)

### 6. Recommendation Helpfulness Count
![Recommendation Helpfulness](FLIPKART%20MBA%20VISUALS/Recommend%20Helpful%20Count.png)

### 7. Correlation between Recommendation and Satisfaction
![Correlation between Recommendation and Satisfaction](FLIPKART%20MBA%20VISUALS/Correlation%20Plot%20Between%20Recommendation%20and%20Satisfaction.png)

### 8. Top Association Rules Heatmap
![Top Association Rules](FLIPKART%20MBA%20VISUALS/Top%20Association%20Rules.png)

### 9. Pairplot of Key Behavioural Drivers
![Pairplot of Key Behavioural Drivers](FLIPKART%20MBA%20VISUALS/Pairplot%20of%20Key%20Behavioural%20Drivers.png)

### 10. How Reviews Affect Ratings (Heatmap)
![Reviews Affect Ratings](FLIPKART%20MBA%20VISUALS/Reviews%20Affect%20Ratings%20Heatmap.png)

### Customer Segmentation
Three clear segments were identified:
- **Frequent Buyers** (High frequency + High satisfaction)
- **Occasional Shoppers** (Medium frequency + Moderate satisfaction)
- **At-Risk Customers** (Low frequency/satisfaction + high cart abandonment)

### Satisfaction & Recommendation Insights
- Overall Shopping Satisfaction is moderate (~3.25/5).
- Personalized recommendations are only **somewhat helpful** for most customers.
- Customers trust real reviews more than Flipkart’s recommendations.



<h1 align="center">Hi 👋, I'm Rishav Singh</h1>
<h3 align="center">A passionate data analyst trainee from India</h3>

<p align="left"> <img src="https://komarev.com/ghpvc/?username=rishav0542&label=Profile%20views&color=0e75b6&style=flat" alt="rishav0542" /> </p>


- 🔭 I’m currently working on **Data Analytics Projects**

- 🌱 I’m currently learning **Data Science and it's core domain**

- 💬 Ask me about **Data and insights!**

- 📫 How to reach me **singh.rishav086@gmail.com**

<h3 align="left">Connect with me:</h3>
<p align="left">
<a href="https://linkedin.com/in/https://www.linkedin.com/in/rishav-singh-11720b298/" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" alt="https://www.linkedin.com/in/rishav-singh-11720b298/" height="30" width="40" /></a>
<a href="https://www.topcoder.com/members/221005" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/topcoder.svg" alt="221005" height="30" width="40" /></a>
</p>

<h3 align="left">Languages and Tools:</h3>
<p align="left"> <a href="https://www.mysql.com/" target="_blank" rel="noreferrer"> <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original-wordmark.svg" alt="mysql" width="40" height="40"/> </a> <a href="https://pandas.pydata.org/" target="_blank" rel="noreferrer"> <img src="https://raw.githubusercontent.com/devicons/devicon/2ae2a900d2f041da66e950e4d48052658d850630/icons/pandas/pandas-original.svg" alt="pandas" width="40" height="40"/> </a> <a href="https://www.python.org" target="_blank" rel="noreferrer"> <img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" alt="python" width="40" height="40"/> </a> <a href="https://scikit-learn.org/" target="_blank" rel="noreferrer"> <img src="https://upload.wikimedia.org/wikipedia/commons/0/05/Scikit_learn_logo_small.svg" alt="scikit_learn" width="40" height="40"/> </a> <a href="https://seaborn.pydata.org/" target="_blank" rel="noreferrer"> <img src="https://seaborn.pydata.org/_images/logo-mark-lightbg.svg" alt="seaborn" width="40" height="40"/> </a> </p>
